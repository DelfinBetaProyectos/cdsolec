  ...
  <div class="max-w-7xl mx-auto mb-14 mt-6 lg:mt-8 sm:px-6 lg:px-8">
    <nav class="mb-3 px-3 py-2 rounded bg-gray-200 text-gray-600">
      <ol class="flex flex-wrap">
        ...
        <li><span class="mx-2">/</span>Editar</li>
      </ol>
    </nav>
    ...
    <div class="flex flex-wrap justify-center">
      <div class="w-full lg:w-3/4 shadow rounded-md overflow-hidden bg-white">
        ...
        <form id="form-update" name="form-update" method="POST" action="{{ route('users.update', $user) }}" enctype="multipart/form-data">
          @csrf
          {{ method_field('PUT') }}
          ...
          <div class="px-4 py-5 grid gap-3 grid-cols-1 md:grid-cols-2">
            ...
            <div class="w-full">
              ...
              <x-jet-input type="text" id="first_name" name="first_name" :value="old('first_name', $user->first_name)" autocomplete="first_name" required />
            ...
            <div class="w-full">
              ...
              <x-jet-input type="text" id="last_name" name="last_name" :value="old('last_name', $user->last_name)" required />
            ...
            <div class="w-full">
              ...
              <div class="relative rounded shadow-sm">
                <div class="absolute inset-y-0 flex items-center">
                  <select id="letter" name="letter" class="form-select h-full py-0 pl-2 pr-7 border-transparent bg-transparent text-gray-500 sm:text-sm sm:leading-5">
                    <option value="V" {{ ($user->identification[0] == 'V') ? 'selected' : '' }}>V</option>
                    <option value="E" {{ ($user->identification[0] == 'E') ? 'selected' : '' }}>E</option>
                    <option value="J" {{ ($user->identification[0] == 'J') ? 'selected' : '' }}>J</option>
                    <option value="G" {{ ($user->identification[0] == 'G') ? 'selected' : '' }}>G</option>
                  </select>
                </div>
                <x-jet-input type="text" id="number" name="number" :value="old('number', substr($user->identification, 1, strlen($user->identification)))" placeholder="Cédula o RIF" class="pl-11 w-full" required />
              </div>
              <x-jet-input type="hidden" id="identification" name="identification" :value="old('identification', $user->identification)" />
            ...
            <div class="w-full">
              ...
              <label for="male" class="inline-flex items-center cursor-pointer mb-2">
                <input type="radio" id="male" name="gender" class="form-radio text-gray-800 w-5 h-5" value="M" wire:model.defer="state.gender" {{ (old('gender', $user->gender) == 'M') ? 'checked' : '' }} />
              ...
              <label for="female" class="inline-flex items-center cursor-pointer mb-2">
                <input type="radio" id="female" name="gender" class="form-radio text-gray-800 w-5 h-5" value="F" wire:model.defer="state.gender" {{ (old('gender', $user->gender) == 'F') ? 'checked' : '' }} />
              ...
              <label for="other" class="inline-flex items-center cursor-pointer mb-2">
                <input type="radio" id="other" name="gender" class="form-radio text-gray-800 w-5 h-5" value="O" wire:model.defer="state.gender" {{ (old('gender', $user->gender) == 'O') ? 'checked' : '' }} />
            ...
            <div class="w-full">
              ...
              <x-jet-input type="email" id="email" name="email" class="mt-1 block w-full" :value="old('email', $user->email)" required />
            ...
            <div class="w-full">
              ...
              <x-jet-input type="text" id="phone" name="phone" class="mt-1 block w-full" :value="old('phone', $user->phone)" pattern="^\(\d{3}\)-\d{3}-\d{4}$" />
            ...
            <fieldset>
              ...
              @foreach($roles as $role)
                <label for="role{{ $role->id }}" class="inline-flex items-center cursor-pointer mb-2">
                  <input type="radio" id="role{{ $role->id }}" name="role" class="form-radio text-cdsolec-green-dark w-5 h-5" value="{{ $role->name }}" {{ ($user->isRol($role->name)) ? 'checked' : '' }} />
